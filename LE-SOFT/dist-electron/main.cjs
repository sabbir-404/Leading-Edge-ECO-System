...
function createFetchClient(options) {
  ...
  return {
    async request({
      ...
      const url = buildUrl(options.baseUrl, path6, query);
...
var CONFIG_PATH = import_path.default.join(import_electron.app.getPath("userData"), "supabase-config.json");
var DEFAULTS = {
...
var supabase = createClient(config.url, config.anonKey, {
  auth: {
    ...
    // Electron handles its own sessions
    autoRefreshToken: true
  },
  global: {
    headers: {
...
var supabaseAdmin = config.serviceRoleKey ? createClient(config.url, config.serviceRoleKey, {
  auth: {
...
var initDB = async () => {
  const { error } = await supabase_default.from("companies").select("id").limit(1);
  if (error) {
...
function getMachineId() {
  const idFile = import_path2.default.join(import_electron2.app.getPath("userData"), ".device-id");
  ...
  const id = (0, import_crypto.createHash)("sha256").update(raw).digest("hex").substring(0, 32);
...
async function registerDevice(username) {
  const { version: version3 } = JSON.parse(
    import_fs2.default.readFileSync(import_path2.default.join(import_electron2.app.getAppPath(), "package.json"), "utf-8")
  );
  const payload = {
    device_id: DEVICE_ID,
    ...
    app_version: version3,
    last_seen: (/* @__PURE__ */ new Date()).toISOString(),
    ...username ? { username } : {}
  };
  const { error } = await supabase_default.from("device_sessions").upsert(payload, { onConflict: "device_id" });
  if (error) {
...
async function setBackupNode(isBackup) {
  const { error } = await supabase_default.from("device_sessions").update({ is_backup_node: isBackup }).eq("device_id", DEVICE_ID);
...
async function getConnectedDevices() {
  const { data, error } = await supabase_default.from("device_sessions").select("*").order("last_seen", { ascending: false });
  if (error) {
...
function startHeartbeat(intervalMs = 6e4) {
  return setInterval(async () => {
    await supabase_default.from("device_sessions").update({ last_seen: (/* @__PURE__ */ new Date()).toISOString() }).eq("device_id", DEVICE_ID);
  }, intervalMs);
}
...
function getLicenseFilePath() {
  ...
  return import_path3.default.join(userDataPath, "license.json");
}
function getMachineId2() {
  ...
  const hash = import_crypto2.default.createHmac("sha256", MACHINE_ID_SALT).update(raw).digest("hex").substring(0, 12).toUpperCase();
...
function validateLicense(machineId, licenseKey) {
  ...
  const verificationHash = import_crypto2.default.createHmac("sha256", VERIFICATION_SALT).update(cleanKey + machineId).digest("hex");
  const expectedPrefix = import_crypto2.default.createHmac("sha256", VERIFICATION_SALT).update(machineId).digest("hex").substring(0, 8).toUpperCase();
...
function isLicensed() {
  ...
  try {
    if (import_fs3.default.existsSync(licensePath)) {
      const data = JSON.parse(import_fs3.default.readFileSync(licensePath, "utf-8"));
...
function saveLicense(key) {
  ...
  try {
    ...
    import_fs3.default.writeFileSync(licensePath, JSON.stringify(data, null, 2), "utf-8");
    return { success: true };
  } catch (e) {
...
function checkBruteForce(username) {
  ...
  const minutesSince = (Date.now() - rec.lastAttempt) / 6e4;
...
var mysqlPool = null;
try {
  const cfgPath = import_path4.default.join(import_electron4.app.getPath("userData"), "mysql-config.json");
...
async function syncProductToMySQL(p) {
  if (!mysqlPool) return;
  const { localId, name, sku, category, sellingPrice, description, imagePath, quantity } = p;
  const [[existing]] = await mysqlPool.query("SELECT id FROM products WHERE le_local_id = ?", [localId]);
  if (existing) {
...
async function syncStockToMySQL(localId, soldQty) {
  if (!mysqlPool) return;
...
var userTyping = /* @__PURE__ */ new Map();
async function writeAuditLog(params) {
  try {
    await supabase_default.from("system_audit_log").insert({
...
function registerHandlers() {
  import_electron4.ipcMain.handle("ping-supabase", async () => {
    try {
  ...
  import_electron4.ipcMain.handle("update-user-presence", async (_e, userId) => {
    userPresence.set(userId, Date.now());
    return { success: true };
  });
  import_electron4.ipcMain.handle("get-online-users", async () => {
    const now = Date.now();
    const onlineIds = [];
    for (const [uid, lastSeen] of userPresence.entries()) {
      if (now - lastSeen < 15e3) onlineIds.push(uid);
      else userPresence.delete(uid);
    }
    return onlineIds;
  });
  import_electron4.ipcMain.handle("set-typing-status", async (_e, { senderId, receiverId, isTyping }) => {
    const key = `${senderId}->${receiverId}`;
    if (isTyping) userTyping.set(key, Date.now());
    else userTyping.delete(key);
    return { success: true };
  });
  import_electron4.ipcMain.handle("get-typing-status", async (_e, { receiverId }) => {
    const now = Date.now();
    const typingIds = [];
  ...
  import_electron4.ipcMain.handle("get-chat-messages", async (_e, { senderId, receiverId }) => {
    const { data, error } = await supabase_default.from("internal_messages").select("*").or(`and(sender_id.eq.${senderId},receiver_id.eq.${receiverId}),and(sender_id.eq.${receiverId},receiver_id.eq.${senderId})`).order("created_at", { ascending: true });
    if (error) throw error;
    return data || [];
  });
  import_electron4.ipcMain.handle("send-chat-message", async (_e, msg) => {
    const { senderId, receiverId, messageType, content, fileName } = msg;
    const { data, error } = await supabase_default.from("internal_messages").insert({
      sender_id: senderId,
  ...
  import_electron4.ipcMain.handle("email-get-inbox", async (_e, userId) => {
    const { data, error } = await supabase_default.from("system_emails").select("*, sender:sender_id(full_name, email)").eq("receiver_id", userId).eq("is_deleted_by_receiver", false).order("created_at", { ascending: false });
    if (error) throw error;
    return data || [];
  });
  import_electron4.ipcMain.handle("email-get-sent", async (_e, userId) => {
    const { data, error } = await supabase_default.from("system_emails").select("*, receiver:receiver_id(full_name, email)").eq("sender_id", userId).eq("is_deleted_by_sender", false).order("created_at", { ascending: false });
    if (error) throw error;
    return data || [];
  });
  import_electron4.ipcMain.handle("email-send", async (_e, emailPayload) => {
  ...
  import_electron4.ipcMain.handle("email-mark-read", async (_e, emailId) => {
    const { error } = await supabase_default.from("system_emails").update({ is_read: true }).eq("id", emailId);
    if (error) throw error;
    return { success: true };
  });
  import_electron4.ipcMain.handle("email-delete", async (_e, { emailId, folder }) => {
  ...
  import_electron4.ipcMain.handle("get-groups", async () => {
    const { data, error } = await supabase_default.from("groups").select("*, parent:groups!parent_group_id(name)").order("name");
    if (error) throw error;
    return (data || []).map((g) => ({ ...g, parent_name: g.parent?.name || null }));
  });
  import_electron4.ipcMain.handle("create-group", async (_e, group) => {
    const { name, parent, nature } = group;
    let parentId = null;
    if (parent && parent !== "Primary") {
      const { data: data2 } = await supabase_default.from("groups").select("id").eq("name", parent).maybeSingle();
      parentId = data2?.id || null;
    }
    const { data, error } = await supabase_default.from("groups").insert({ name, parent_group_id: parentId, nature: nature || null, company_id: 1 }).select("id").single();
    if (error) throw error;
    return { success: true, id: data.id };
  });
  import_electron4.ipcMain.handle("delete-group", async (_e, id) => {
    const { error } = await supabase_default.from("groups").delete().eq("id", id);
    if (error) throw error;
    return { success: true };
  });
  import_electron4.ipcMain.handle("get-ledgers", async () => {
    const { data, error } = await supabase_default.from("ledgers").select("*, group:groups(name)").order("name");
    if (error) throw error;
    return (data || []).map((l) => ({ ...l, group_name: l.group?.name || null }));
  });
  import_electron4.ipcMain.handle("create-ledger", async (_e, ledger) => {
    ...
    const { data, error } = await supabase_default.from("ledgers").insert({ name, group_id: grp?.id || null, opening_balance: openingBalance || 0, opening_balance_type: type || "Dr", mailing_name: mailingName || "", address: address || "", tax_reg_no: gstin || "", company_id: 1 }).select("id").single();
    if (error) throw error;
    return { success: true, id: data.id };
  });
  import_electron4.ipcMain.handle("delete-ledger", async (_e, id) => {
    const { error } = await supabase_default.from("ledgers").delete().eq("id", id);
    if (error) throw error;
    return { success: true };
  });
  import_electron4.ipcMain.handle("get-vouchers", async () => {
    const { data, error } = await supabase_default.from("vouchers").select("*, voucher_entries(ledger_id, amount, type, ledger:ledgers(name))").order("date", { ascending: false }).order("id", { ascending: false });
    if (error) throw error;
    return (data || []).map((v) => ({ ...v, particulars: v.voucher_entries?.map((e) => e.ledger?.name).filter(Boolean).join(", ") || "" }));
  });
  import_electron4.ipcMain.handle("create-voucher", async (_e, voucher) => {
    ...
    const totalAmount = rows.reduce((s, r) => s + Math.max(Number(r.debit) || 0, Number(r.credit) || 0), 0) / 2;
  ...
  import_electron4.ipcMain.handle("delete-voucher", async (_e, id) => {
  ...
  import_electron4.ipcMain.handle("get-units", async () => {
    const { data, error } = await supabase_default.from("units").select("*").order("name");
    if (error) throw error;
    return data || [];
  });
  import_electron4.ipcMain.handle("create-unit", async (_e, unit) => {
    const { data, error } = await supabase_default.from("units").insert({ name: unit.name, symbol: unit.symbol, precision: unit.precision || 0, company_id: 1 }).select("id").single();
    if (error) throw error;
    return { success: true, id: data.id };
  });
  import_electron4.ipcMain.handle("delete-unit", async (_e, id) => {
    const { error } = await supabase_default.from("units").delete().eq("id", id);
    if (error) throw error;
    return { success: true };
  });
  import_electron4.ipcMain.handle("get-stock-groups", async () => {
    const { data, error } = await supabase_default.from("stock_groups").select("*, parent:stock_groups!parent_id(name)").order("name");
    if (error) throw error;
    return (data || []).map((g) => ({ ...g, parent_name: g.parent?.name || null }));
  });
  import_electron4.ipcMain.handle("create-stock-group", async (_e, group) => {
    let parentId = null;
    if (group.parent && group.parent !== "Primary") {
      const { data: data2 } = await supabase_default.from("stock_groups").select("id").eq("name", group.parent).maybeSingle();
      parentId = data2?.id || null;
    }
    const { data, error } = await supabase_default.from("stock_groups").insert({ name: group.name, parent_id: parentId, company_id: 1 }).select("id").single();
    if (error) throw error;
    return { success: true, id: data.id };
  });
  import_electron4.ipcMain.handle("delete-stock-group", async (_e, id) => {
    const { error } = await supabase_default.from("stock_groups").delete().eq("id", id);
    if (error) throw error;
    return { success: true };
  });
  import_electron4.ipcMain.handle("get-stock-items", async () => {
    const { data, error } = await supabase_default.from("stock_items").select("*, group:stock_groups(name), unit:units(name,symbol)").order("name");
    if (error) throw error;
    return (data || []).map((i) => ({ ...i, group_name: i.group?.name || null, unit_name: i.unit?.name || null, unit_symbol: i.unit?.symbol || null }));
  });
  import_electron4.ipcMain.handle("create-stock-item", async (_e, item) => {
    ...
    const { data, error } = await supabase_default.from("stock_items").insert({ name, group_id: gRow?.id || null, unit_id: uRow?.id || null, opening_qty: openingQty || 0, opening_rate: openingRate || 0, opening_value: openValue, company_id: 1 }).select("id").single();
    if (error) throw error;
    return { success: true, id: data.id };
  });
  import_electron4.ipcMain.handle("delete-stock-item", async (_e, id) => {
    const { error } = await supabase_default.from("stock_items").delete().eq("id", id);
    if (error) throw error;
    return { success: true };
  });
  import_electron4.ipcMain.handle("get-companies", async () => {
    const { data, error } = await supabase_default.from("companies").select("*").order("name");
    if (error) throw error;
    return data || [];
  });
  import_electron4.ipcMain.handle("create-company", async (_e, c) => {
    const { data, error } = await supabase_default.from("companies").insert({ name: c.name, mailing_name: c.mailingName || c.name, address: c.address || "", country: c.country || "Bangladesh", state: c.state || "", phone: c.phone || "", email: c.email || "", financial_year_from: c.financialYearFrom || "", books_begin_from: c.booksBeginFrom || "", base_currency_symbol: c.currencySymbol || "\u09F3" }).select("id").single();
    if (error) throw error;
    return { success: true, id: data.id };
  });
  import_electron4.ipcMain.handle("get-dashboard-stats", async () => {
    ...
    return {
      ...
      totalTransactions: (vouchers.data || []).reduce((s, v) => s + (v.total_amount || 0), 0),
      stockItemCount: stockItems.count || 0,
      productCount: products.count || 0,
      totalRevenue: (bills.data || []).reduce((s, b) => s + (b.grand_total || 0), 0),
      recentVouchers: recentV.data || []
    };
  });
  import_electron4.ipcMain.handle("get-products", async () => {
    const { data, error } = await supabase_default.from("products").select("*, unit:units(name,symbol), group:stock_groups(name)").order("name");
    if (error) throw error;
    return (data || []).map((p) => ({ ...p, unit_name: p.unit?.name || null, unit_symbol: p.unit?.symbol || null, group_name: p.group?.name || null }));
  });
  import_electron4.ipcMain.handle("get-product", async (_e, id) => {
    const { data, error } = await supabase_default.from("products").select("*, unit:units(name,symbol), group:stock_groups(name)").eq("id", id).maybeSingle();
    if (error) throw error;
    if (!data) return null;
    return { ...data, unit_name: data.unit?.name || null, unit_symbol: data.unit?.symbol || null, group_name: data.group?.name || null };
  });
  import_electron4.ipcMain.handle("create-product", async (_e, product) => {
  ...
  import_electron4.ipcMain.handle("update-product", async (_e, product) => {
    ...
    const { data: uRow } = await supabase_default.from("units").select("id").eq("name", unit).maybeSingle();
    const { data: gRow } = await supabase_default.from("stock_groups").select("id").eq("name", stockGroup).maybeSingle();
    const { error } = await supabase_default.from("products").update({ name, sku: sku || "", category: category || "", purchase_price: purchasePrice || 0, selling_price: sellingPrice || 0, tax_rate: taxRate || 0, hsn_code: hsnCode || "", description: description || "", unit_id: uRow?.id || null, stock_group_id: gRow?.id || null, image_path: imagePath || "", quantity: quantity || 0 }).eq("id", id);
    if (error) throw error;
  ...
  import_electron4.ipcMain.handle("delete-product", async (_e, id) => {
    const { error } = await supabase_default.from("products").delete().eq("id", id);
    if (error) throw error;
    return { success: true };
  });
  import_electron4.ipcMain.handle("search-products-detailed", async (_e, query) => {
    const q = `%${query}%`;
    const { data, error } = await supabase_default.from("products").select("*, unit:units(symbol), group:stock_groups(name)").or(`name.ilike.${q},sku.ilike.${q},category.ilike.${q}`).limit(20);
    if (error) throw error;
    return (data || []).map((p) => ({ ...p, unit_symbol: p.unit?.symbol || null, group_name: p.group?.name || null }));
  });
  import_electron4.ipcMain.handle("search-billing-customers", async (_e, query) => {
    const q = `%${query}%`;
    const { data } = await supabase_default.from("billing_customers").select("*").or(`phone.ilike.${q},name.ilike.${q}`).order("name").limit(10);
    return data || [];
  });
  import_electron4.ipcMain.handle("create-billing-customer", async (_e, customer) => {
    ...
    if (phone) {
      ...
      if (existing) {
        await supabase_default.from("billing_customers").update({ name: name || existing.name, email: email || existing.email, address: address || existing.address }).eq("id", existing.id);
        return { ...existing, name: name || existing.name };
      }
    }
    const { data, error } = await supabase_default.from("billing_customers").insert({ name, phone: phone || null, email: email || "", address: address || "", total_bills: 0 }).select("*").single();
    if (error) throw error;
    return data;
  });
  import_electron4.ipcMain.handle("create-bill", async (_e, billData) => {
    const { customer_id, billed_by, items, subtotal, discount_total, grand_total } = billData;
    const now = /* @__PURE__ */ new Date();
    const dateStr = now.getFullYear().toString() + String(now.getMonth() + 1).padStart(2, "0") + String(now.getDate()).padStart(2, "0");
    const { data: cust } = await supabase_default.from("billing_customers").select("phone").eq("id", customer_id).maybeSingle();
    const phoneLast4 = cust?.phone ? cust.phone.slice(-4) : "0000";
    const { count } = await supabase_default.from("bills").select("id", { count: "exact", head: true }).ilike("invoice_number", `${dateStr}%`);
    const serial = String((count || 0) + 1).padStart(3, "0");
    const invoiceNumber = `${dateStr}-${phoneLast4}-${serial}`;
    const { data: bill, error: bErr } = await supabase_default.from("bills").insert({ invoice_number: invoiceNumber, customer_id, billed_by: billed_by || "Admin", subtotal, discount_total, grand_total }).select("id").single();
    if (bErr) throw bErr;
    if (items.length > 0) {
      await supabase_default.from("bill_items").insert(items.map((item) => ({ bill_id: bill.id, product_id: item.product_id, product_name: item.product_name, sku: item.sku || "", quantity: item.quantity, mrp: item.mrp, discount_pct: item.discount_pct || 0, discount_amt: item.discount_amt || 0, price: item.price })));
      for (const item of items) {
        if (item.product_id && item.quantity) {
          try {
            await supabase_default.rpc("decrement_product_qty", { p_id: item.product_id, qty: item.quantity });
          } catch {
          }
    ...
    try {
      await supabase_default.from("billing_customers").update({ total_bills: supabase_default.sql`total_bills + 1` }).eq("id", customer_id);
    } catch {
    }
    return { success: true, id: bill.id, invoice_number: invoiceNumber };
  });
  import_electron4.ipcMain.handle("get-bills", async () => {
    const { data, error } = await supabase_default.from("bills").select("*, customer:billing_customers(name,phone)").order("created_at", { ascending: false });
    if (error) throw error;
    return (data || []).map((b) => ({ ...b, customer_name: b.customer?.name || null, customer_phone: b.customer?.phone || null }));
  });
  import_electron4.ipcMain.handle("get-bill-details", async (_e, billId) => {
    ...
    return { ...bill, customer_name: bill.customer?.name, customer_phone: bill.customer?.phone, customer_email: bill.customer?.email, customer_address: bill.customer?.address, items: (items || []).map((i) => ({ ...i, image_path: i.product?.image_path || null })) };
  });
  import_electron4.ipcMain.handle("update-bill", async (_e, billData) => {
    ...
    if (auditRows.length) await supabase_default.from("bill_audit").insert(auditRows);
    for (const oi of oldItems || []) {
      try {
        await supabase_default.from("products").update({ quantity: supabase_default.sql`quantity + ${oi.quantity}` }).eq("id", oi.product_id);
      } catch {
      }
    }
    await supabase_default.from("bills").update({ subtotal, discount_total, grand_total }).eq("id", bill_id);
    await supabase_default.from("bill_items").delete().eq("bill_id", bill_id);
    if (items.length) {
      await supabase_default.from("bill_items").insert(items.map((i) => ({ bill_id, product_id: i.product_id, product_name: i.product_name, sku: i.sku || "", quantity: i.quantity, mrp: i.mrp, discount_pct: i.discount_pct || 0, discount_amt: i.discount_amt || 0, price: i.price })));
      for (const i of items) {
        try {
          await supabase_default.from("products").update({ quantity: supabase_default.sql`GREATEST(quantity - ${i.quantity}, 0)` }).eq("id", i.product_id);
        } catch {
        }
      }
    }
  ...
  import_electron4.ipcMain.handle("get-bill-audit", async (_e, billId) => {
    const { data } = await supabase_default.from("bill_audit").select("*").eq("bill_id", billId).order("changed_at", { ascending: false });
    return data || [];
  });
  import_electron4.ipcMain.handle("get-purchase-bills", async () => {
    const { data, error } = await supabase_default.from("purchase_bills").select("*, supplier:ledgers(name)").order("bill_date", { ascending: false }).order("id", { ascending: false });
    if (error) throw error;
    return (data || []).map((b) => ({ ...b, supplier_name: b.supplier?.name || null }));
  });
  import_electron4.ipcMain.handle("create-purchase-bill", async (_e, bill) => {
    const { billNumber, billDate, dueDate, supplierLedgerId, narration, items } = bill;
    let subtotal = 0, taxTotal = 0;
    (items || []).forEach((item) => {
      const la = (Number(item.qty) || 0) * (Number(item.rate) || 0);
      subtotal += la;
      taxTotal += la * ((Number(item.taxRate) || 0) / 100);
    });
    const { data: pb, error } = await supabase_default.from("purchase_bills").insert({ bill_number: billNumber, bill_date: billDate, due_date: dueDate || "", supplier_ledger_id: supplierLedgerId || null, narration: narration || "", subtotal, tax_total: taxTotal, grand_total: subtotal + taxTotal, company_id: 1 }).select("id").single();
    if (error) throw error;
    if ((items || []).length) {
      await supabase_default.from("purchase_bill_items").insert(items.map((item) => {
        const la = (Number(item.qty) || 0) * (Number(item.rate) || 0);
        const lt = la * ((Number(item.taxRate) || 0) / 100);
  ...
  import_electron4.ipcMain.handle("delete-purchase-bill", async (_e, id) => {
  ...
  import_electron4.ipcMain.handle("change-password", async (_e, data) => {
    ...
    const { data: row } = await supabase_default.from("users").select("id,password_hash").eq("id", id).maybeSingle();
    if (!row) return { success: false, error: "User not found" };
    const stored = row.password_hash || "";
    let matches = stored.startsWith("$2b$") || stored.startsWith("$2a$") ? await import_bcrypt.default.compare(currentPassword, stored) : stored === currentPassword;
    ...
    await supabase_default.from("users").update({ password_hash: newHash }).eq("id", id);
    return { success: true };
  });
  import_electron4.ipcMain.handle("get-users", async () => {
    const { data, error } = await supabase_default.from("users").select("id,username,full_name,role,email,phone,is_active,created_at").order("created_at", { ascending: false });
    if (error) throw error;
    return data || [];
  });
  import_electron4.ipcMain.handle("create-user", async (_e, user) => {
    ...
    const { error: updateErr } = await supabase_default.from("users").update({
  ...
  import_electron4.ipcMain.handle("update-user", async (_e, user) => {
    const { id, fullName, role, email, phone, isActive } = user;
    const { error } = await supabase_default.from("users").update({ full_name: fullName, role, email: email || "", phone: phone || "", is_active: isActive !== void 0 ? isActive : 1 }).eq("id", id);
    if (error) throw error;
    return { success: true };
  });
  import_electron4.ipcMain.handle("delete-user", async (_e, id) => {
    const { data: row } = await supabase_default.from("users").select("auth_id").eq("id", id).single();
  ...
  import_electron4.ipcMain.handle("authenticate-user", async (_e, credentials) => {
    ...
    const { data: authData, error: authError } = await supabase_default.auth.signInWithPassword({
      email: emailToUse,
      password
    });
    ...
    const { data: row } = await supabase_default.from("users").select("*").eq("auth_id", authData.user.id).single();
    if (!row) {
  ...
  import_electron4.ipcMain.handle("get-settings", async () => {
    const { data } = await supabase_default.from("companies").select("*").eq("id", 1).maybeSingle();
    return data || {};
  });
  import_electron4.ipcMain.handle("update-settings", async (_e, s) => {
    const { error } = await supabase_default.from("companies").update({ name: s.name, mailing_name: s.mailingName || "", address: s.address || "", country: s.country || "Bangladesh", state: s.state || "", phone: s.phone || "", email: s.email || "", financial_year_from: s.financialYearFrom || "", books_begin_from: s.booksBeginFrom || "", base_currency_symbol: s.currencySymbol || "\u09F3" }).eq("id", 1);
    if (error) throw error;
    return { success: true };
  });
  import_electron4.ipcMain.handle("save-supabase-config", async (_e, newConfig) => {
    try {
      const currentConfigPath = import_path4.default.join(import_electron4.app.getPath("userData"), "supabase-config.json");
      let configToSave = {
        url: "https://ildkkgjrolcjijwfokek.supabase.co",
        anonKey: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImlsZGtrZ2pyb2xjamlqd2Zva2VrIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzE5MzMzMjQsImV4cCI6MjA4NzUwOTMyNH0.Bn6c-87BOumPXyH5F469P04fQSMnI9SjNDZAwgGyTsM",
        serviceRoleKey: newConfig.serviceRoleKey || ""
      };
      if (import_fs4.default.existsSync(currentConfigPath)) {
        const existing = JSON.parse(import_fs4.default.readFileSync(currentConfigPath, "utf8"));
        configToSave = { ...existing, ...configToSave };
      }
      import_fs4.default.writeFileSync(currentConfigPath, JSON.stringify(configToSave, null, 2), "utf8");
      return { success: true };
    } catch (e) {
      return { success: false, error: e.message };
    }
  });
  import_electron4.ipcMain.handle("get-user-groups", async () => {
    const { data, error } = await supabase_default.from("user_groups").select("*").order("id");
    if (error) throw error;
    return data || [];
  });
  import_electron4.ipcMain.handle("create-user-group", async (_e, group) => {
    const { data, error } = await supabase_default.from("user_groups").insert({ name: group.name, description: group.description || "", permissions: JSON.stringify(group.permissions || {}) }).select("id").single();
    if (error) throw error;
    return { success: true, id: data.id };
  });
  import_electron4.ipcMain.handle("update-user-group", async (_e, group) => {
    const { error } = await supabase_default.from("user_groups").update({ name: group.name, description: group.description || "", permissions: JSON.stringify(group.permissions || {}) }).eq("id", group.id);
    if (error) throw error;
    return { success: true };
  });
  import_electron4.ipcMain.handle("delete-user-group", async (_e, id) => {
    const { error } = await supabase_default.from("user_groups").delete().eq("id", id);
    if (error) throw error;
    return { success: true };
  });
  import_electron4.ipcMain.handle("get-notifications", async (_e, userId) => {
    const { data, error } = await supabase_default.from("notifications").select("*, sender:users!sender_id(full_name)").or(`recipient_id.eq.${userId},recipient_id.is.null`).order("created_at", { ascending: false }).limit(100);
    if (error) throw error;
    return (data || []).map((n) => ({ ...n, sender_name: n.sender?.full_name || null }));
  });
  import_electron4.ipcMain.handle("send-notification", async (_e, notification) => {
    const { title, message, senderId, recipientIds } = notification;
    if (!recipientIds || recipientIds.length === 0) {
      const { data, error: error2 } = await supabase_default.from("notifications").insert({ title, message, sender_id: senderId, recipient_id: null }).select("id").single();
      if (error2) throw error2;
      return { success: true, id: data.id };
    }
    const rows = recipientIds.map((rid) => ({ title, message, sender_id: senderId, recipient_id: rid }));
    const { error } = await supabase_default.from("notifications").insert(rows);
    if (error) throw error;
    return { success: true, count: recipientIds.length };
  });
  import_electron4.ipcMain.handle("mark-notification-read", async (_e, id) => {
    await supabase_default.from("notifications").update({ is_read: true }).eq("id", id);
    return { success: true };
  });
  import_electron4.ipcMain.handle("mark-all-notifications-read", async (_e, userId) => {
  ...
  import_electron4.ipcMain.handle("delete-notification", async (_e, id) => {
    await supabase_default.from("notifications").delete().eq("id", id);
    return { success: true };
  });
  import_electron4.ipcMain.handle("pick-image", async () => {
    const result = await import_electron4.dialog.showOpenDialog({ properties: ["openFile"], filters: [{ name: "Images", extensions: ["jpg", "jpeg", "png", "gif", "webp", "bmp"] }] });
    if (result.canceled || result.filePaths.length === 0) return null;
    return result.filePaths[0];
  });
  import_electron4.ipcMain.handle("pick-chat-file", async () => {
    const result = await import_electron4.dialog.showOpenDialog({ properties: ["openFile"], filters: [{ name: "All Files", extensions: ["*"] }, { name: "Images", extensions: ["jpg", "png", "gif", "webp"] }, { name: "Docs", extensions: ["pdf", "doc", "docx", "xls", "xlsx", "txt"] }] });
    if (result.canceled || result.filePaths.length === 0) return null;
    return { path: result.filePaths[0], name: import_path4.default.basename(result.filePaths[0]) };
  });
  import_electron4.ipcMain.handle("report-trial-balance", async () => {
    ...
    for (const e of entries || []) {
      ...
      if (e.type === "Dr") entryMap[e.ledger_id].dr += e.amount;
      else entryMap[e.ledger_id].cr += e.amount;
    }
  ...
  import_electron4.ipcMain.handle("report-balance-sheet", async () => {
    const { data } = await supabase_default.from("ledgers").select("id,name,opening_balance,opening_balance_type,group:groups!inner(name,nature)").in("groups.nature", ["Assets", "Liabilities"]).order("name");
    const { data: entries } = await supabase_default.from("voucher_entries").select("ledger_id,amount,type");
    const em = {};
    for (const e of entries || []) {
      if (!em[e.ledger_id]) em[e.ledger_id] = { dr: 0, cr: 0 };
      if (e.type === "Dr") em[e.ledger_id].dr += e.amount;
      else em[e.ledger_id].cr += e.amount;
    }
    return (data || []).map((l) => ({ ...l, group_name: l.group?.name, nature: l.group?.nature, total_debit: em[l.id]?.dr || 0, total_credit: em[l.id]?.cr || 0 }));
  });
  import_electron4.ipcMain.handle("report-profit-and-loss", async () => {
    const { data } = await supabase_default.from("ledgers").select("id,name,opening_balance,opening_balance_type,group:groups!inner(name,nature)").in("groups.nature", ["Income", "Expenses"]).order("name");
    const { data: entries } = await supabase_default.from("voucher_entries").select("ledger_id,amount,type");
    const em = {};
    for (const e of entries || []) {
      if (!em[e.ledger_id]) em[e.ledger_id] = { dr: 0, cr: 0 };
  ...
  import_electron4.ipcMain.handle("report-stock-summary", async () => {
    ...
    const pbiMap = {};
    for (const i of pbi || []) {
      if (!pbiMap[i.product_id]) pbiMap[i.product_id] = { qty: 0, value: 0 };
      pbiMap[i.product_id].qty += i.qty || 0;
      pbiMap[i.product_id].value += i.amount || 0;
    }
  ...
  import_electron4.ipcMain.handle("report-day-book", async (_e, params) => {
    ...
    if (error) throw error;
    return (data || []).flatMap(
      (v) => (v.voucher_entries || []).map((e) => ({ id: v.id, voucher_type: v.voucher_type, voucher_number: v.voucher_number, date: v.date, narration: v.narration, total_amount: v.total_amount, entry_amount: e.amount, entry_type: e.type, ledger_name: e.ledger?.name || null }))
    );
  });
  import_electron4.ipcMain.handle("get-make-orders", async () => {
    const { data, error } = await supabase_default.from("make_orders").select("*").order("created_at", { ascending: false });
    if (error) throw error;
    return data || [];
  });
  import_electron4.ipcMain.handle("create-make-order", async (_e, order) => {
    const { data, error } = await supabase_default.from("make_orders").insert({ furniture_name: order.furniture_name, description: order.description || "", quantity: order.quantity || 1, designer_name: order.designer_name, status: "Placed", priority: order.priority || "Normal" }).select("id").single();
    if (error) throw error;
    await supabase_default.from("make_order_updates").insert({ order_id: data.id, status: "Placed", note: "Order placed", updated_by: order.designer_name });
    return { id: data.id };
  });
  import_electron4.ipcMain.handle("update-make-order-status", async (_e, { orderId, status, note, updatedBy }) => {
  ...
  import_electron4.ipcMain.handle("get-make-order-updates", async (_e, orderId) => {
    const { data } = await supabase_default.from("make_order_updates").select("*").eq("order_id", orderId).order("created_at");
    return data || [];
  });
  import_electron4.ipcMain.handle("delete-make-order", async (_e, id) => {
  ...
  import_electron4.ipcMain.handle("get-make-furniture-names", async () => {
    const { data } = await supabase_default.from("make_orders").select("furniture_name").order("furniture_name");
    return [...new Set((data || []).map((r) => r.furniture_name))];
  });
  import_electron4.ipcMain.handle("get-printers", async () => {
    try {
      const mainWin = import_electron4.BrowserWindow.getAllWindows()[0];
      if (!mainWin) return [];
  ...
  import_electron4.ipcMain.handle("get-audit-log", async (_e, { module: module2, limit }) => {
  ...
  import_electron4.ipcMain.handle("stage-bill-alteration", async (_e, { billId, changes, reason, changedBy }) => {
    ...
    const { data, error } = await supabase_default.from("bill_audit").insert({ bill_id: billId, field_changed: "items", old_value: JSON.stringify(snapshot), staged_data: JSON.stringify(changes), alter_reason: reason || "", alter_status: "pending_approval", changed_by: changedBy }).select("id").single();
    if (error) throw error;
  ...
  import_electron4.ipcMain.handle("get-pending-alterations", async () => {
    const { data, error } = await supabase_default.from("bill_audit").select("*, bill:bills(invoice_number, customer:billing_customers(name))").eq("alter_status", "pending_approval").order("changed_at", { ascending: false });
    if (error) throw error;
    return (data || []).map((r) => ({ ...r, invoice_number: r.bill?.invoice_number, customer_name: r.bill?.customer?.name }));
  });
  import_electron4.ipcMain.handle("approve-alteration", async (_e, { auditId, reviewedBy }) => {
    ...
    await supabase_default.from("bill_items").delete().eq("bill_id", audit.bill_id);
    if (items.length) {
      await supabase_default.from("bill_items").insert(items.map((i) => ({ bill_id: audit.bill_id, product_id: i.product_id || null, product_name: i.product_name, sku: i.sku || "", quantity: i.quantity, mrp: i.mrp, discount_pct: i.discount_pct || 0, discount_amt: i.discount_amt || 0, price: i.price })));
      const subtotal = items.reduce((s, i) => s + i.mrp * i.quantity, 0);
      const discountTotal = items.reduce((s, i) => s + (i.discount_amt || 0), 0);
      await supabase_default.from("bills").update({ subtotal, discount_total: discountTotal, grand_total: staged.grand_total || subtotal - discountTotal }).eq("id", audit.bill_id);
    }
  ...
  import_electron4.ipcMain.handle("reject-alteration", async (_e, { auditId, reviewedBy, rejectReason }) => {
  ...
  import_electron4.ipcMain.handle("add-bill-shipping", async (_e, data) => {
    const { data: sh, error } = await supabase_default.from("bill_shipping").upsert({ bill_id: data.bill_id, ship_to_name: data.ship_to_name, ship_to_address: data.ship_to_address, ship_to_phone: data.ship_to_phone || "", ship_from_name: data.ship_from_name || "", ship_from_address: data.ship_from_address || "", shipping_charge: data.shipping_charge || 0, updated_by: data.updated_by, status: "pending_payment" }, { onConflict: "bill_id" }).select("id").single();
    if (error) throw error;
    await supabase_default.from("shipping_status_log").insert({ shipment_id: sh.id, bill_id: data.bill_id, status: "pending_payment", note: "Shipping order created", updated_by: data.updated_by, updated_by_role: data.user_role || "cashier" });
  ...
  import_electron4.ipcMain.handle("get-bill-shipping", async (_e, billId) => {
    const { data } = await supabase_default.from("bill_shipping").select("*").eq("bill_id", billId).maybeSingle();
    return data || null;
  });
  import_electron4.ipcMain.handle("get-all-shipments", async (_e, { status } = {}) => {
    ...
    const { data, error } = await q;
    if (error) throw error;
    return (data || []).map((s) => ({ ...s, invoice_number: s.bill?.invoice_number, bill_date: s.bill?.created_at, customer_name: s.bill?.customer?.name, customer_phone: s.bill?.customer?.phone }));
  });
  import_electron4.ipcMain.handle("get-shipment-history", async (_e, shipmentId) => {
    const { data } = await supabase_default.from("shipping_status_log").select("*").eq("shipment_id", shipmentId).order("created_at");
    return data || [];
  });
  import_electron4.ipcMain.handle("update-shipment-status", async (_e, { shipmentId, billId, status, note, updatedBy, userRole, imagePath }) => {
  ...
  import_electron4.ipcMain.handle("upload-packaging-image", async (_e, { shipmentId, billId, imageBase64, updatedBy, userRole }) => {
    try {
      const imgDir = import_path4.default.join(import_electron4.app.getPath("userData"), "packaging_images");
      if (!import_fs4.default.existsSync(imgDir)) import_fs4.default.mkdirSync(imgDir, { recursive: true });
      const filename = `ship_${shipmentId}_${Date.now()}.jpg`;
      const imgPath = import_path4.default.join(imgDir, filename);
      import_fs4.default.writeFileSync(imgPath, Buffer.from(imageBase64.replace(/^data:image\/\w+;base64,/, ""), "base64"));
  ...
  import_electron4.ipcMain.handle("get-machine-id", async () => getMachineId2());
  import_electron4.ipcMain.handle("check-license", async () => isLicensed());
  import_electron4.ipcMain.handle("activate-license", async (_e, key) => {
    if (!key || typeof key !== "string" || key.trim().length < 10) return { success: false, error: "Invalid license key format" };
    return saveLicense(key.trim());
  });
  import_electron4.ipcMain.handle("get-db-monitoring", async () => {
    const [products, bills, users, vouchers, makeOrders] = await Promise.all([
      ...
      supabase_default.from("users").select("id", { count: "exact", head: true }),
      supabase_default.from("vouchers").select("id", { count: "exact", head: true }),
      supabase_default.from("make_orders").select("id", { count: "exact", head: true })
    ]);
  ...
  function getBackupDir() {
    const d = import_path4.default.join(import_electron4.app.getPath("userData"), "backups");
    if (!import_fs4.default.existsSync(d)) import_fs4.default.mkdirSync(d, { recursive: true });
    return d;
  }
  import_electron4.ipcMain.handle("create-db-backup", async () => {
    try {
      ...
      for (const t of tables) {
        const { data } = await supabase_default.from(t).select("*");
        backup[t] = data || [];
      }
      const ts = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-").substring(0, 19);
      const backupFile = import_path4.default.join(getBackupDir(), `le-soft-backup-${ts}.json`);
      import_fs4.default.writeFileSync(backupFile, JSON.stringify(backup, null, 2), "utf-8");
      const files = import_fs4.default.readdirSync(getBackupDir()).filter((f) => f.startsWith("le-soft-backup-") && f.endsWith(".json")).sort().reverse();
      files.slice(10).forEach((f) => {
        try {
          import_fs4.default.unlinkSync(import_path4.default.join(getBackupDir(), f));
        } catch {
        }
      });
  ...
  import_electron4.ipcMain.handle("list-db-backups", async () => {
    try {
      return import_fs4.default.readdirSync(getBackupDir()).filter((f) => f.startsWith("le-soft-backup-") && f.endsWith(".json")).map((f) => {
        const st = import_fs4.default.statSync(import_path4.default.join(getBackupDir(), f));
  ...
  import_electron4.ipcMain.handle("restore-db-backup", async (_e, backupName) => {
    try {
      const backupPath = import_path4.default.join(getBackupDir(), backupName);
      if (!import_fs4.default.existsSync(backupPath)) return { success: false, error: "Backup file not found" };
      const backup = JSON.parse(import_fs4.default.readFileSync(backupPath, "utf-8"));
      for (const [table, rows] of Object.entries(backup)) {
        if (!Array.isArray(rows) || rows.length === 0) continue;
        try {
          await supabase_default.from(table).upsert(rows);
        } catch {
        }
      }
  ...
  import_electron4.ipcMain.handle("import-woocommerce-csv", async (_e, csvFilePath) => {
    try {
      let raw = import_fs4.default.readFileSync(csvFilePath, "utf-8");
      ...
      let unitId;
      const { data: uRow } = await supabase_default.from("units").select("id").or(`symbol.eq.pcs,name.eq.Pieces`).maybeSingle();
      if (uRow) {
        unitId = uRow.id;
      } else {
        const { data: nu } = await supabase_default.from("units").insert({ name: "Pieces", symbol: "pcs", precision: 0, company_id: 1 }).select("id").single();
        unitId = nu.id;
      }
      const { data: existingGroups } = await supabase_default.from("stock_groups").select("id,name");
      const categoryMap = /* @__PURE__ */ new Map();
      (existingGroups || []).forEach((g) => categoryMap.set(g.name.toLowerCase(), g.id));
      const allCats = /* @__PURE__ */ new Set();
      for (let i = 1; i < lines.length; i++) {
        const f = parseCSVLine(lines[i]);
        if (iCategory >= 0 && f[iCategory]) {
          f[iCategory].split("|").forEach((c) => c.split(">").forEach((p) => {
            if (p.trim()) allCats.add(p.trim());
          }));
        }
      }
      for (const cat of allCats) {
        if (!categoryMap.has(cat.toLowerCase())) {
          const { data: ng } = await supabase_default.from("stock_groups").insert({ name: cat, company_id: 1 }).select("id").single();
          if (ng) categoryMap.set(cat.toLowerCase(), ng.id);
        }
      }
      ...
      for (let i = 1; i < lines.length; i++) {
        try {
          ...
          const pType = iType >= 0 ? f[iType]?.trim().toLowerCase() : "";
          if (pType === "variation") {
            skipped++;
            continue;
          }
          ...
          let imagePath = "";
          if (iImages >= 0 && f[iImages]) {
            imagePath = f[iImages].trim().split("|")[0].trim().split("!")[0].trim();
          }
          const category = iCategory >= 0 ? (f[iCategory]?.trim() || "").replace(/\|/g, ", ") : "";
          let stockGroupId = null;
          if (iCategory >= 0 && f[iCategory]) {
            const first = f[iCategory].split("|")[0].split(">").map((s) => s.trim()).filter(Boolean);
            const primary = first[first.length - 1];
            if (primary && categoryMap.has(primary.toLowerCase())) stockGroupId = categoryMap.get(primary.toLowerCase()) || null;
          }
      ...
      if (insertRows.length) {
        const { error } = await supabase_default.from("products").insert(insertRows);
        if (error) return { imported: 0, skipped: lines.length - 1, errors: [error.message] };
      }
  ...
  import_electron4.ipcMain.handle("sync-products-to-website", async () => {
    ...
    for (const row of rows) {
      try {
        await syncProductToMySQL({ localId: row.id, name: row.name, sku: row.sku || "", category: row.category || "", sellingPrice: row.selling_price || 0, description: row.description || "", imagePath: row.image_path || "", quantity: row.quantity || 0 });
        synced++;
      } catch {
        failed++;
      }
  ...
  import_electron4.ipcMain.handle("get-connected-devices", async () => getConnectedDevices());
  import_electron4.ipcMain.handle("set-backup-node", async (_e, isBackup) => setBackupNode(isBackup));
  import_electron4.ipcMain.handle("get-device-id", async () => DEVICE_ID);
  import_electron4.ipcMain.handle("get-app-version", async () => import_electron4.app.getVersion());
  import_electron4.ipcMain.handle("restart-app", () => {
  ...
  try {
    ...
    const broadcast = (payload) => import_electron4.BrowserWindow.getAllWindows().forEach((w) => w.webContents.send("update-status", payload));
    ...
    autoUpdater.on("error", (err) => broadcast({ status: "error", error: err?.message || "Update failed" }));
    setTimeout(() => {
      try {
  ...
  import_electron4.ipcMain.handle("get-network-config", async () => ({}));
  import_electron4.ipcMain.handle("save-network-config", async () => ({ success: true }));
  import_electron4.ipcMain.handle("test-server-connection", async () => ({ success: false, error: "Network config not applicable in Supabase mode" }));
  import_electron4.ipcMain.handle("get-local-ip", async () => {
  ...
  import_electron4.ipcMain.handle("make-upload-pdf", async (_e, { orderId, filePath }) => {
    let filesToUpload = [];
    if (filePath) {
    ...
    for (const p of filesToUpload) {
      const fileName = import_path4.default.basename(p);
      const fileBuffer = import_fs4.default.readFileSync(p);
  ...
  import_electron4.ipcMain.handle("make-get-pdf-urls", async (_e, orderId) => {
    const { data: order } = await supabase_default.from("make_orders").select("pdf_urls").eq("id", orderId).maybeSingle();
    const paths = order?.pdf_urls || [];
    const signedUrls = await Promise.all(paths.map(async (p) => {
      const { data } = await supabase_default.storage.from("make-order-files").createSignedUrl(p, 3600);
      return { path: p, name: import_path4.default.basename(p).replace(/^\d+_/, ""), url: data?.signedUrl || "" };
    }));
    return signedUrls.filter((u) => u.url);
  });
  import_electron4.ipcMain.handle("make-delete-pdf", async (_e, { orderId, storagePath }) => {
  ...
  import_electron4.ipcMain.handle("make-download-pdf", async (_e, { url, fileName }) => {
    try {
      const https = await import("https");
      const http = await import("http");
      const tmpPath = import_path4.default.join(import_electron4.app.getPath("temp"), fileName);
      await new Promise((resolve, reject) => {
        const file = import_fs4.default.createWriteStream(tmpPath);
  ...
  import_electron4.ipcMain.handle("make-get-order-parts", async (_e, orderId) => {
    const { data } = await supabase_default.from("make_order_parts").select("*").eq("order_id", orderId).order("sort_order").order("id");
    return data || [];
  });
  import_electron4.ipcMain.handle("make-upsert-part", async (_e, part) => {
    if (part.id) {
      const { data, error } = await supabase_default.from("make_order_parts").update({ part_name: part.part_name, length: part.length, width: part.width, height: part.height, notes: part.notes, sort_order: part.sort_order }).eq("id", part.id).select().maybeSingle();
      if (error) return { error: error.message };
      return data;
    } else {
      const { data, error } = await supabase_default.from("make_order_parts").insert({ order_id: part.order_id, part_name: part.part_name, length: part.length || "", width: part.width || "", height: part.height || "", notes: part.notes || "", sort_order: part.sort_order || 0 }).select().maybeSingle();
      if (error) return { error: error.message };
      return data;
    }
  });
  import_electron4.ipcMain.handle("make-delete-part", async (_e, partId) => {
    const { error } = await supabase_default.from("make_order_parts").delete().eq("id", partId);
    return error ? { error: error.message } : { success: true };
  });
  const ALTERABLE_BY_DESIGNER = ["Placed"];
  const ALTERABLE_FIELDS = ["furniture_name", "description", "quantity", "priority"];
  import_electron4.ipcMain.handle("make-alter-order", async (_e, {
    orderId,
    changes,
    alteredBy,
    userRole
  }) => {
    const { data: current, error: fetchErr } = await supabase_default.from("make_orders").select("*").eq("id", orderId).maybeSingle();
    ...
    await supabase_default.from("make_order_alteration_log").insert(logRows);
    const { error: updateErr } = await supabase_default.from("make_orders").update(filteredChanges).eq("id", orderId);
    if (updateErr) return { error: updateErr.message };
    return { success: true, changed: Object.keys(filteredChanges) };
  });
  import_electron4.ipcMain.handle("make-get-alteration-log", async (_e, orderId) => {
    const { data } = await supabase_default.from("make_order_alteration_log").select("*").eq("order_id", orderId).order("altered_at", { ascending: false });
    return data || [];
  });
  import_electron4.ipcMain.handle("make-get-dashboard-stats", async () => {
  ...
  import_electron4.ipcMain.handle("hrm-get-employees", async () => {
    const { data, error } = await supabase_default.from("hrm_employees").select("*").order("name");
    if (error) throw error;
    return data || [];
  });
  import_electron4.ipcMain.handle("hrm-upsert-employee", async (_e, emp) => {
    if (emp.id) {
      const { error } = await supabase_default.from("hrm_employees").update(emp).eq("id", emp.id);
      if (error) throw error;
      return { success: true };
    } else {
      const { data, error } = await supabase_default.from("hrm_employees").insert(emp).select("id").single();
      if (error) throw error;
      return { success: true, id: data.id };
    }
  });
  import_electron4.ipcMain.handle("hrm-delete-employee", async (_e, id) => {
    const { error } = await supabase_default.from("hrm_employees").delete().eq("id", id);
    if (error) throw error;
    return { success: true };
  });
  import_electron4.ipcMain.handle("hrm-get-attendance", async (_e, { date }) => {
  ...
  import_electron4.ipcMain.handle("hrm-mark-attendance", async (_e, att) => {
  ...
  import_electron4.ipcMain.handle("hrm-get-leaves", async () => {
    const { data, error } = await supabase_default.from("hrm_leaves").select("*, employee:employee_id(name)").order("created_at", { ascending: false });
    if (error) throw error;
    return data || [];
  });
  import_electron4.ipcMain.handle("hrm-request-leave", async (_e, leave) => {
    const { data, error } = await supabase_default.from("hrm_leaves").insert(leave).select("id").single();
    if (error) throw error;
    return { success: true, id: data.id };
  });
  import_electron4.ipcMain.handle("hrm-update-leave-status", async (_e, { id, status }) => {
    const { error } = await supabase_default.from("hrm_leaves").update({ status }).eq("id", id);
    if (error) throw error;
    return { success: true };
  });
  import_electron4.ipcMain.handle("hrm-get-payroll", async (_e, { month, year }) => {
  ...
  import_electron4.ipcMain.handle("hrm-generate-payroll", async (_e, pr) => {
  ...
  import_electron4.ipcMain.handle("hrm-mark-payroll-paid", async (_e, id) => {
  ...
  import_electron4.ipcMain.handle("crm-get-customers", async () => {
    const { data, error } = await supabase_default.from("crm_customers").select("*").order("name");
    if (error) throw error;
    return data || [];
  });
  import_electron4.ipcMain.handle("crm-upsert-customer", async (_e, cust) => {
    if (cust.id) {
      const { error } = await supabase_default.from("crm_customers").update(cust).eq("id", cust.id);
      if (error) throw error;
      return { success: true };
    } else {
      const { data, error } = await supabase_default.from("crm_customers").insert(cust).select("id").single();
      if (error) throw error;
      return { success: true, id: data.id };
    }
  });
  import_electron4.ipcMain.handle("crm-get-tracking-logs", async (_e, { customerId }) => {
    const { data, error } = await supabase_default.from("crm_tracking").select("*, user:user_id(full_name)").eq("customer_id", customerId).order("created_at", { ascending: false });
    if (error) throw error;
    return data || [];
  });
  import_electron4.ipcMain.handle("crm-add-tracking-log", async (_e, log) => {
    const { data, error } = await supabase_default.from("crm_tracking").insert(log).select("id").single();
    if (error) throw error;
    return { success: true, id: data.id };
  });
}
...
function createWindow() {
  ...
  const win = new import_electron5.BrowserWindow({
    ...
    icon: import_path5.default.join(__dirname, "../build/icon.ico"),
  ...
  if (import_electron5.app.isPackaged) {
    win.loadFile(import_path5.default.join(__dirname, "../dist/index.html"));
  } else {
