...
function registerHandlers() {
    ...
    if (items.length > 0) {
      ...
      for (const item of items) {
        if (item.product_id && item.quantity) {
          await supabase_default.rpc("decrement_product_qty", { p_id: item.product_id, qty: item.quantity }).catch(() => {
          });
          syncStockToMySQL(item.product_id, item.quantity);
        }
      }
    }
    await supabase_default.from("billing_customers").update({ total_bills: supabase_default.sql`total_bills + 1` }).eq("id", customer_id).catch(() => {
    });
  ...
  import_electron4.ipcMain.handle("update-bill", async (_e, billData) => {
    ...
    for (const oi of oldItems || []) {
      await supabase_default.from("products").update({ quantity: supabase_default.sql`quantity + ${oi.quantity}` }).eq("id", oi.product_id).catch(() => {
      });
    }
    ...
    if (items.length) {
      ...
      for (const i of items) {
        await supabase_default.from("products").update({ quantity: supabase_default.sql`GREATEST(quantity - ${i.quantity}, 0)` }).eq("id", i.product_id).catch(() => {
        });
      }
    }
    return { success: true };
  ...
  import_electron4.ipcMain.handle("create-user", async (_e, user) => {
    ...
    const hash = await import_bcrypt.default.hash(password, BCRYPT_ROUNDS);
    const { data, error } = await supabase_default.from("users").insert({ username: username.trim(), password_hash: hash, full_name: fullName || "", role: role || "operator", email: email || "", phone: phone || "" }).select("id").single();
  ...
  import_electron4.ipcMain.handle("authenticate-user", async (_e, credentials) => {
    ...
    const { data: row } = await supabase_default.from("users").select("id,username,password_hash,full_name,role,email,is_active").eq("username", username).eq("is_active", 1).maybeSingle();
    if (!row) {
      recordFailedLogin(username);
      await import_bcrypt.default.hash("dummy", 4);
      return { success: false, error: "Invalid credentials or account disabled" };
    }
    const stored = row.password_hash || "";
    let valid = stored.startsWith("$2b$") || stored.startsWith("$2a$") ? await import_bcrypt.default.compare(password, stored) : stored === password;
    if (!valid && !(stored.startsWith("$2b$") || stored.startsWith("$2a$")) && stored === password) {
      const newHash = await import_bcrypt.default.hash(password, BCRYPT_ROUNDS);
      await supabase_default.from("users").update({ password_hash: newHash }).eq("id", row.id);
      valid = true;
    }
    if (!valid) {
      recordFailedLogin(username);
  ...
  import_electron4.ipcMain.handle("get-chat-messages", async (_e, { senderId, receiverId }) => {
    const { data, error } = await supabase_default.from("internal_messages").select("*").or(`and(sender_id.eq.${senderId},receiver_id.eq.${receiverId}),and(sender_id.eq.${receiverId},receiver_id.eq.${senderId})`).order("created_at");
    if (error) throw error;
    return data || [];
  });
  import_electron4.ipcMain.handle("send-chat-message", async (_e, message) => {
    const { senderId, receiverId, messageType, content, fileName } = message;
    const { data, error } = await supabase_default.from("internal_messages").insert({ sender_id: senderId, receiver_id: receiverId, message_type: messageType || "text", content, file_name: fileName || null }).select("id,created_at").single();
    if (error) throw error;
  ...
  import_electron4.ipcMain.handle("get-online-users", async () => {
    const now = Date.now();
    const onlineIds = [];
    userPresence.forEach((ts, uid) => {
      if (now - ts < 15e3) onlineIds.push(uid);
    });
    return onlineIds;
  });
  ...
  import_electron4.ipcMain.handle("get-typing-status", async (_e, { receiverId }) => {
    const now = Date.now();
    const typingUsers = [];
    userTyping.forEach((ts, key) => {
      const [sender, recv] = key.split("->").map(Number);
      if (recv === receiverId && now - ts < 3e3) typingUsers.push(sender);
    });
    return typingUsers;
  });
  ...
  import_electron4.ipcMain.handle("restore-db-backup", async (_e, backupName) => {
    try {
      ...
      for (const [table, rows] of Object.entries(backup)) {
        ...
        await supabase_default.from(table).upsert(rows).catch(() => {
        });
      }
  ...
  import_electron4.ipcMain.handle("set-backup-node", async (_e, deviceId) => setBackupNode(deviceId));
